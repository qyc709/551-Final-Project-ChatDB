...
query_templates = [
    {
        ...
        "query": "SELECT {primary}, {categorical} FROM {table}"
    },
    {
        "caption": "Find {columns} and {agg_func} {agg_column}, where the output is grouped by {group_by}",
        "query": "SELECT {columns}, {agg_func}({agg_column}) FROM {table} GROUP BY {group_by}"
    },
    {
    ...
    },
    {
        ...
        "query": "SELECT {categorical}, {numerical} FROM {table} WHERE {where_col} {operator} {num}"
    },
    {
        "caption": "Find unique {categorical}",
        "query": "SELECT DISTINCT({categorical}) FROM {table}"
    },
    {
        "caption": "Find {columns} and {agg_func} {agg_column}, where the output is grouped by {group_by} and {where_col} is {operator_int} {num}",
        "query": "SELECT {columns}, {agg_func}({agg_column}) FROM {table} WHERE {where_col} {operator} {num} GROUP BY {group_by}"
    },
    {
        "caption": "Find first {n} {columns}",
        "query": "SELECT {columns} FROM {table} LIMIT {n}"
    }
]
...
def tokenize_phrase(column_names):
    ...
    return ", ".join(flattened_column_names)
...
def selected_random_query(user_input, database):
    ...
    elif re.search(r'\b(limit)\b', user_input.lower(), re.IGNORECASE): # add
        template = query_templates[6]
...
def interpret_user_input_generic(user_input, database):
    ...
    if match_num:
        for match in match_num:
            ...
            query_components["n"]=match.group()
    ...
    if re.search(r'\b(group by|groupby)\b', user_input.lower(), re.IGNORECASE) and re.search(r'\b(where|if|with|greater|bigger|larger|smaller|less|fewer)\b', user_input.lower(), re.IGNORECASE):
        ...
        return queries, captions
    elif user_input is not None and re.search(r'\b(first|limit)\b', user_input.lower(), re.IGNORECASE): # add
        queries.append(query_templates[6]["query"].format(**query_components))
        return queries, captions
